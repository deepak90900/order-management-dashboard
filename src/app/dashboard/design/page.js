...
export default function DesignPage() {
  ...
  useEffect(() => {
    // Fetch orders in the "Design" stage from the backend
    const fetchOrders = async () => {
      try {
        const response = await fetch("/api/orders?stage=Design");
        if (!response.ok) {
  ...
  const handleComplete = async (orderId) => {
    ...
    try {
      const response = await fetch(`/api/orders/${orderId}`, {
        ...
        body: JSON.stringify({ stage: "Print" }),
      });
